...
const ProductDetailCollapsed = ({product}) => {
  ...
  const [apiLink, setApiLink] = useState(`${API_BASE}/repairs?prod_id=${product.id}&tech_id=${username}`);

  // update apiLink on selected 
  useEffect(()=> {
    let isComponentMounted = true

    if (isComponentMounted) {
      setApiLink(`${API_BASE}/repairs?prod_id=${product.id}&tech_id=${username}`);
    }
     return () => {
       isComponentMounted = false;
     }
  }, [API_BASE, product, username]);
  ...
  const handleStartRepair = async () => {
    ...
    if (response.ok) {
      SuccessToast(`Repair started for ${responseText.id}`);
